...
Guia de referência criado a partir da publicação do site **edithsantos.adv.br** (Astro + Tailwind), para reaproveitar em próximos sites.
...
Registro.br ──(nameservers)──► Cloudflare DNS
                                   │
                                   ├─ edithsantos.adv.br      → Pages (principal)
                                   └─ www.edithsantos.adv.br  → redirect 301 → domínio principal
                                   │
                                   ▼
...
2. Adicione o domínio raiz (ex.: `edithsantos.adv.br`) e confirme. A Cloudflare cria o CNAME sozinha.
