...
const DIST = "dist"
const SITE = "https://edithsantos.adv.br"
...
for (const { rota, html, noindex } of paginas) {
  ...
  if (!/<html[^>]*lang="pt-BR"/.test(html))
    falha(rota, 'falta <html lang="pt-BR">')
  ...
  if (descricao.length < 120 || descricao.length > 155)
  ...
  if (meta(html, 'property="og:locale"') !== "pt_BR")
    falha(rota, "og:locale deve ser pt_BR")
  ...
  for (const [, tag, conteudo] of html.matchAll(/(<a\b[^>]*>)(.*?)<\/a>/gs)) {
    ...
    if (/^(clique aqui|aqui|saiba mais|leia mais|veja mais|mais)$/.test(texto))
