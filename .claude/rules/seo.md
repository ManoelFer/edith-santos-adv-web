...
- **Open Graph e Twitter:** `og:title`, `og:description`, `og:url`, `og:type`, `og:image` (1200×630, URL absoluta), `og:locale` (`pt_BR`) e `twitter:card` = `summary_large_image`. O layout aceita uma prop `image` opcional com imagem padrão do site.
- `<html lang="pt-BR">`, `charset` e `viewport` sempre presentes.
...
- Nome de arquivo descritivo em kebab-case (`advogada-atendimento.jpg`, não `IMG_2034.jpg`).
...
- Crédito de quem desenvolveu o site ("Desenvolvido por ...") fica **só no rodapé**, discreto, com link.
...
- Profissões regulamentadas (advocacia, saúde, contabilidade etc.) seguem as regras de publicidade do conselho, em um arquivo próprio em `.claude/rules/` (ex.: [`publicidade-advocacia.md`](publicidade-advocacia.md)). Elas têm prioridade sobre qualquer texto "vendedor".
